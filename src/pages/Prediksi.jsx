    ...
    const fetchData = async () => {
      try {
        ...
        const [
          ...
          api.get(
            "/transactions/summary/monthly?range=365d&groupBy=month"
          ),
        ...
        const predIncome =
          Number(forecastData?.pred_income) || 0;

        const predExpense =
          Number(forecastData?.pred_expense) || 0;

        const forecastRecommendation =
          forecastData?.recommendation?.trim() ||
          "Belum ada rekomendasi AI";

        // Build a meaningful insight string from forecast fields
        const buildForecastAnalysis = (fd) => {
          ...
          const currentSavingsRate = fd.current_savings_rate != null
            ? `${(fd.current_savings_rate * 100).toFixed(1)}%`
            : null;

          const predSavingsRate = fd.pred_savings_rate != null
            ? `${(fd.pred_savings_rate * 100).toFixed(1)}%`
            : null;
          ...
          if (currentSavingsRate || predSavingsRate) {
            lines.push(
              `Savings rate saat ini: ${currentSavingsRate ?? "-"} → prediksi bulan depan: ${predSavingsRate ?? "-"}`
            );
          }
      ...
      <div className="container-fluid">
        ...
        >
          ...
          <span className="fw-semibold text-dark">
            Silahkan isi data pemasukan dan
            pengeluaran anda minimal 3 bulan
              ...
              <div className="card-body p-4">
                ...
                >
                  <div className="d-flex align-items-center justify-content-between">
                    ...
                    >
                      Rp{" "}
                      {prediction.predIncome.toLocaleString(
                        "id-ID"
                      )}
                    </span>
                  </div>
              ...
              <div className="card-body p-4">
                ...
                >
                  <div className="d-flex align-items-center justify-content-between">
                    ...
                    >
                      Rp{" "}
                      {prediction.predExpense.toLocaleString(
                        "id-ID"
                      )}
                    </span>
                  </div>
